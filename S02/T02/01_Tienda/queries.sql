...
SELECT nombre, precio AS '€', precio*1.08 AS '$' FROM producto;
...
SELECT p.nombre AS 'nombre producto', p.codigo AS 'codigo producto', f.nombre AS 'nombre fabricante', f.codigo AS 'codigo fabricante'
FROM producto p, fabricante f
WHERE p.codigo_fabricante = f.codigo;

-- 24. Retorna el nom del producte, el seu preu i el nom del seu fabricant, del producte més barat.
SELECT p.nombre, p.precio AS precio, f.nombre AS fabricante
FROM producto p, fabricante f
WHERE p.codigo_fabricante = f.codigo
ORDER BY precio ASC
LIMIT 1;
...
SELECT f.nombre AS fabricante, p.nombre
FROM fabricante f
...
WHERE p.precio = (SELECT MAX(precio) FROM producto p, fabricante f
	WHERE p.codigo_fabricante = f.codigo AND f.nombre = 'Lenovo' GROUP BY f.nombre);

-- 38. Llista el nom del producte més car del fabricant Lenovo.
SELECT p.nombre, p.precio FROM producto p, fabricante f
WHERE p.precio = (SELECT MAX(precio) FROM producto p, fabricante f
	WHERE p.codigo_fabricante = f.codigo AND f.nombre = 'Lenovo' GROUP BY f.nombre)
    AND p.codigo_fabricante = f.codigo AND f.nombre = 'Lenovo';

-- 39. Llista el nom del producte més barat del fabricant Hewlett-Packard.
SELECT p.nombre FROM producto p, fabricante f
WHERE p.precio = (SELECT MIN(precio) FROM producto p, fabricante f
	WHERE p.codigo_fabricante = f.codigo AND f.nombre = 'Hewlett-Packard' GROUP BY f.nombre)
AND p.codigo_fabricante = f.codigo AND f.nombre = 'Hewlett-Packard';
...
WHERE p.precio >= (SELECT AVG(precio) FROM producto p, fabricante f
	WHERE p.codigo_fabricante = f.codigo AND f.nombre = 'Asus' GROUP BY f.nombre)
AND p.codigo_fabricante = f.codigo AND f.nombre = 'Asus';