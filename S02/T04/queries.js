...
{ $project: { restaurant_id: true, name: true, borough: true, cuisine: true } }]);
...
db.restaurants.aggregate([
    {
        "$match": {
            "$and": [
                { "grades.1.score": { "$gt": 42 } },
                { "grades.1.score": { "$lt": 52 } }
            ]
        }
    },
    {
...
db.restaurants.find({ "name": { "$regex": /mon/ } },
    { name: true, borough: true, "address.coord": true, cuisine: true, _id: false });
