...
db.restaurants.find({ "grades.score": { "$gt": 70 }, "address.coord.0": { "$lt": -65.754168 }, "cuisine": { "$not": { $regex: /American/i } } });
...
db.restaurants.find({ "grades.score": { "$gt": 70 }, "address.coord.0": { "$lt": -65.754168 }, "cuisine": { "$not": { "$regex": "American" } } });
...
db.restaurants.find({ "grades.grade": "A", borough: { "$not": { "$eq": 'Brooklyn' } }, "cuisine": { "$not": { "$regex": "American" } } }).sort({ cuisine: 1 })
...
db.restaurants.aggregate([{ "$match": { "$and": [{ "$or": [{ "cuisine": { "$regex": /American/i } }, { "cuisine": { "$regex": /Chinese/i } }] }, { "borough": "Bronx" }] } }]);
...
db.restaurants.aggregate([{ "$match": { "$or": [{ borough: "Staten Island" }, { borough: "Queens" }, { borough: "Bronx" }, { borough: "Brooklyn" },] } }, { $project: { restaurant_id: true, name: true, borough: true, cuisine: true } }]);
...
db.restaurants.aggregate([{ "$match": { "$nor": [{ borough: "Staten Island" }, { borough: "Queens" }, { borough: "Bronx" }, { borough: "Brooklyn" },] } }, { $project: { restaurant_id: true, name: true, borough: true, cuisine: true } }]);
...
db.restaurants.aggregate([{ "$match": { "$or": [{ "$and": [{ "cuisine": { "$regex": /seafood/i } }, { "$nor": [{ "cuisine": { "$regex": /American/i } }, { "cuisine": { "$regex": /Chinese/i } }] }] }, { "name": { "$regex": /^Wil/i } }] } }, { "$project": { "restaurant_id": true, "name": true, "borough": true, "cuisine": true, "_id": 0 } }]);
...
db.restaurants.find({ "grades.grade": "A", "grades.score": 11, "grades.date": ISODate("2014-08-11T00:00:00Z") }, { restaurant_id: true, name: true, grades: true })
...
db.restaurants.find({ "grades.1.grade": "A", "grades.1.score": 9, "grades.date": ISODate("2014-08-11T00:00:00Z") }, { restaurant_id: true, name: true, grades: true })
...
db.restaurants.aggregate([{ "$match": { "$and": [{ "grades.1.score": { "$gt": 42 } }, { "grades.1.score": { "$lt": 52 } }] } }, { "$project": { restaurant_id: true, name: true, address: true, borough: true } }]);
...
db.restaurants.find({}, { name: true, cuisine: true, borough: true, _id: false }).sort({ cuisine: 1, borough: -1 })

// 28. Escriu una consulta per saber tant si totes les direccions contenen el carrer o no.
db.restaurants.find({ "address.street": { "$exists": true, "$ne": null } }).count() == db.restaurants.find().count()
...
db.restaurants.aggregate([{ "$match": { "$or": [{ "address.coord.0": { $type: 1 } }, { "address.coord.1": { $type: 1 } }] } }, { "$project": { name: true, "address.coord": true } }]);
...
db.restaurants.aggregate([{ "$match": { "grades.score": { $mod: [7, 0] } } }, { "$project": { restaurant_id: true, name: true, grades: true } }]);
...
db.restaurants.find({ "name": { "$regex": /mon/ } }, { name: true, borough: true, "address.coord": true, cuisine: true, _id: false });
...
db.restaurants.find({ "name": { "$regex": /^Mad/ } }, { name: true, borough: true, "address.coord": true, cuisine: true, _id: false });